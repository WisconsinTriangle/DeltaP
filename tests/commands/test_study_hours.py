...
class TestStudyHoursCommand:
    ...
    @pytest.mark.asyncio
    async def test_study_hours_checks_role(self, sample_env_vars):
        """Test that study_hours command checks for Brother role."""
        mock_bot = Mock()
        mock_bot.tree = Mock()
        ...
        # Test with check_brother_role returning False
        with patch('role.role_checking.check_brother_role', new_callable=AsyncMock) as mock_role_check:
            mock_role_check.return_value = False

            if study_hours_func:
                await study_hours_func(mock_interaction, "Eli", 5.0)
                mock_interaction.response.send_message.assert_called_once()
                # Check that the message contains permission denial
    ...
    async def test_study_hours_invalid_pledge_name(self, sample_env_vars):
        ...
        with patch('role.role_checking.check_brother_role', new_callable=AsyncMock) as mock_role_check:
            ...
            if study_hours_func:
                await study_hours_func(mock_interaction, "InvalidPledge", 5.0)
                mock_interaction.response.send_message.assert_called_once()
    ...
    async def test_study_hours_successful_submission(self, sample_env_vars):
        ...
        with patch('role.role_checking.check_brother_role', new_callable=AsyncMock) as mock_role_check:
            ...
            if study_hours_func:
                ...
                assert "5.0" in message or "5" in message
